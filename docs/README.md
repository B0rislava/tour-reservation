...
./gradlew bootRun
```
*Алтернативно, може просто да стартирате `TourReservationApplication.java` директно през бутона "Play" във вашата среда (IntelliJ / VS Code).*
...
Преди да тествате API-тата, **задължително се уверете, че Spring Boot приложението е стартирано** (както е описано по-горе).

1. **HTTP API Тестове (`api-tests.http`):**  
   Отворете файла `api-tests.http` чрез вашата среда (IntelliJ IDEA или VS Code). Ще видите малки "Play" (Run) бутони до всяка заявка (GET, POST). Просто ги натиснете, за да изпратите заявка към работещия сървър.

2. **Integration Tests (Java):**  
   В папката `src/test/java/com/spring/tour_reservation/` се намира файлът `TourControllerIntegrationTest.java`. Можете да го стартирате директно от вашата среда ("Run Test"), за да потвърдите, че бекендът валидира заявките коректно чрез `MockMvc`.
...
2. **Tours**
   * `id`, `title`, `description`, `location`, `duration`, `price_per_person`, `max_group_size`, `available_spots`, `meeting_point`, `whats_included`, `scheduled_date`, `start_time`, `status`, `created_at`, `guide_id` (FK).
3. **Bookings**
...
Началната страница посреща потребителите с динамичен списък от всички налични туристически пакети. За всеки тур се показва локация, продължителност и цена, както и бутон за разглеждане на детайли.
...
Тук се предоставя изчерпателна информация за избраната екскурзия – пълно описание, точка на среща, максимален брой хора и бутон за добавяне в любими. Само потребители с роля Traveler имат достъп до формата за резервиране на дати.

![Детайли за тур](tour-details.png)

### 5. Моите резервации (My Bookings)
Списък, от който Пътешественикът (Traveler) проследява направените резервации. Показват се детайли за запазените дати, броя участници и общата сума.
...
Табела за управление, достъпна само за екскурзоводи (Guide). Тук се листват създадените от тях турове и се предоставя функционалност за лесно добавяне, редакция или изтриване.

![Моите турове](my-tours.png)

### 8. Създаване на тур (Create Tour) - *за Guide*
Интуитивна форма, където гидът попълва необходимите данни за нов туристически пакет (заглавие, цена, локация, снимка, дата и час на провеждане). Данните се изпращат към REST API-то на Spring приложението.
...
Страница, показваща личните данни на потребителя с опции за преглед на информацията.
